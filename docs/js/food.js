...
/* ============================================================
   PAST DAY EDITOR (edit meals for a previous day, from the home
   "past days" strip)
   ============================================================ */
function openDayEdit(dateKey){
  state.dayEditTarget = dateKey;
  if(!appState.logs[dateKey]) appState.logs[dateKey] = {meals:[], workouts:[], waterMl:0};
  const d = new Date(dateKey+'T00:00:00');
  document.getElementById('dayEditTitle').textContent = `تعديل وجبات — ${formatDateHuman(d)}`;
  document.getElementById('dayEditFoodSearch').value = '';
  renderDayEditMeals();
  renderDayEditFoodList();
  openSheet('sheetDayEdit');
}

function renderDayEditMeals(){
  const wrap = document.getElementById('dayEditMealsList');
  const dateKey = state.dayEditTarget;
  const dayLog = appState.logs[dateKey] || {meals:[]};
  const meals = dayLog.meals || [];
  if(meals.length===0){ wrap.innerHTML = '<div class="empty-hint">ولا وجبة مسجلة هاليوم بعد</div>'; return; }
  wrap.innerHTML = '';
  const deleteDayMeal = (id)=>{
  ...
  meals.forEach(m=>{
    ...
    row.innerHTML = `<div class="entry-dot" style="background:var(--protein)"></div>
      <div class="entry-main"><div class="t1">${escapeHtml(m.name)}</div><div class="t2">${m.category} · ب${Math.round(m.protein)} ك${Math.round(m.carbs)} د${Math.round(m.fat)}</div></div>
      <div class="entry-side tabular">${m.calories}</div>
      <div class="entry-del" data-del-daymeal="${m.id}">✕</div>`;
    wrap.appendChild(row);
  });
  wrap.querySelectorAll('[data-del-daymeal]').forEach(btn=>{
    btn.addEventListener('click', ()=> deleteDayMeal(btn.getAttribute('data-del-daymeal')));
  });
}

function renderDayEditFoodList(){
  const wrap = document.getElementById('dayEditFoodList');
  const q = (document.getElementById('dayEditFoodSearch').value||'').trim();
  let list = state.library.foods;
  if(q) list = list.filter(f=>f.name.includes(q));
  wrap.innerHTML = '';
  if(list.length===0){ wrap.innerHTML = '<div class="empty-hint">ما فيه نتائج</div>'; return; }
  list.forEach(food=>{
    ...
    row.innerHTML = `<div class="lm"><div class="n">${escapeHtml(food.name)}</div><div class="d">${food.category} · ${food.calories} سعرة · ب${food.protein} ك${food.carbs} د${food.fat}</div></div>
      <div class="lib-add">+</div>`;
    row.querySelector('.lib-add').addEventListener('click', (e)=>{ e.stopPropagation(); addFoodToDay(food); });
    wrap.appendChild(row);
  });
}
...
function addFoodToDay(food){
  ...
  appState.logs[dateKey].meals.push(entry);
  food.usageCount = (food.usageCount||0)+1;
  persist();
  vibrate(10);
  const d = new Date(dateKey+'T00:00:00');
  showToast(`أضيفت ${food.name} ليوم ${formatDateHuman(d)}`);
  renderDayEditMeals();
  renderPastDaysStrip();
}
