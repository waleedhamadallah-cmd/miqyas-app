...
function renderExList(){
  ...
  list.forEach(ex=>{
    ...
    row.innerHTML = `${exAnimHtml(ex,'sm')}<div class="lm"><div class="n">${escapeHtml(ex.name)}${picked?'<span class="pr-badge">مختار</span>':''}</div><div class="d">${ex.group} · ${hint}</div></div><div class="fav-star${ex.favorite?' on':''}" data-fav="${ex.id}">${ex.favorite?'★':'☆'}</div>`;
